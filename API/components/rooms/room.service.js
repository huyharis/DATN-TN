...
exports.join = async ({ user_id, socket_id, topic_id }) => {
  ...
  const room = await _randomRoom({
    user_id,
    socket_id,
    topic_id,
    config_size_room: room_config.size,
  });
  if (!room) {
    // CREATE NEW ROOM
    const [topic, user] = await Promise.all([
      !user_id
        ? topicService.randomTopic().then((topic) => topic._id.toString())
        : Promise.resolve(theme_id),
    ...
    _rooms.set(room_id, {
      ...
      final_rank_credits:
        Array.from(room_config.final_rank_credits) || config.final_rank_credits,
      dicUsers: {
        [user_id]: room_user,
      },
...
const _randomRoom = ({ topic_id, config_size_room }) => {
  const conditions = {
    ...(topic_id ? { topic_id } : {}),
    status: 1,
    _id: {
  ...
  return Room.countDocuments(conditions).then((count) => {
    return Room.findOne(conditions).skip(Math.floor(Math.random() * count));
  });
};
...
const _startRoom = async ({ room, socketRoom, EVENTS }) => {
  ...
  try {
    ...
    if (room.users.length === 1 && !!socketRoom) {
      ...
      _rooms.get(room_id).questions = question._makeQuestion(
        {},
        topic.vocabularies
      );
    ...
    if (room.users.length === room.size) {
      ...
      _rooms
        .get(room_id)
        .broadcast(EVENTS.QUIZ_END_LOADING_PLAYERS, {
          duration: config.roomEndLoadingIns,
        });
      await startCounter(config.roomEndLoadingIns);

      _rooms
        .get(room_id)
        .broadcast(EVENTS.QUIZ_START_COUNTDOWN, {
          duration: config.roomCountdownIns,
        });
      await startCounter(config.roomCountdownIns);

      _rooms
        .get(room_id)
        .broadcast(EVENTS.QUIZ_WAIT_ANIMATION, {
          duration: config.delayAnimation,
        });
    ...
    _rooms
      .get(room_id)
      .broadcast(EVENTS.QUIZ_JOIN_ERROR, { message: error.message });
  }
};
